...
async def card_websocket(websocket: WebSocket, room_id: str, token: str):
    ...
    # 再接続時に切断猶予タイマーをキャンセル
    if user_id in session.disconnect_tasks:
        session.disconnect_tasks[user_id].cancel()
        del session.disconnect_tasks[user_id]
    ...
    try:
        ...
        # 即時切断にせず5秒の待機猶予タスクを起動
        session.disconnect_tasks[user_id] = asyncio.create_task(delayed_disconnect(room_id, user_id, delay=5))

async def delayed_disconnect(room_id: str, user_id: str, delay: int = 5):
    try:
